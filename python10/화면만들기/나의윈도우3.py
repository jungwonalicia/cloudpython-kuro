...
def select_process2(count):
    global id_result
    print(count)
    print("정보 검색 시작..")
    print(id_result.get(count))
#     record = db_select()  
#     select_result_ui(record)
...
def select_all_ui3(result):
    ...
    global id_result, count
    
    for record in result:
        if start == size:
            break
        id_result[count] = Button(w, text=record[0], font=("궁서", 30), fg="blue", bg="lime", command = lambda x : select_process2(id_result[count]['text']) )
